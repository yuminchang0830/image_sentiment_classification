...
model = Sequential()
model.add(Conv2D(32, kernel_size=(3, 3), kernel_initializer='RandomNormal', activation='relu',input_shape=input_shape))
model.add(Conv2D(32, (3, 3), kernel_initializer='RandomNormal', activation='relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
#model.add(Dropout(0.25))
model.add(Conv2D(64, (3, 3), kernel_initializer='RandomNormal', activation='relu'))
model.add(Conv2D(64, (3, 3), kernel_initializer='RandomNormal', activation='relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
#model.add(Dropout(0.25))
model.add(Flatten())
...
datagen = ImageDataGenerator(rotation_range=10, width_shift_range=0.1, height_shift_range=0.1, horizontal_flip=True,data_format='channels_last')
datagen.fit(x_train)
history = model.fit_generator(datagen.flow(x_train, y_train_label, batch_size=batch_size), steps_per_epoch= (len(x_train)*2) / batch_size, epochs=epochs, validation_data=(x_test, y_test_label))
